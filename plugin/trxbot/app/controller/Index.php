...
class Index {
    ...
    public function installtg(Request $request){  
        ...
        if(empty($this->long)){ 
            ...
            try {
                ...
                if(empty($res['code'])){
                    return json(['code' => 0,  'msg' => "域名验证失败1"]);
                } 
                
            } catch (\Throwable $e) {  
                 return json(['code' => 0,  'msg' => "域名验证失败,请确认解析正确且能访问：{$data['WEB_URL']}<br>访问域名会出现：<b>部署正常</b>等字符<br>".$e->getMessage()]);
            }
        }else{
        ...
        try {
            ...
            Cache::delete("trx_{$data['API_BOT']}_setup"); //删除设置缓存
            ...
            $bot_commands = Db::name('bot_commands')->where('bot', $data['API_BOT'])->where('command', "菜单指令消息按钮")->where('parentId',0)->where('type',0)->find();
            if(empty($bot_commands)){
                $bot_commands['bot'] = $data['API_BOT'];  
                $bot_commands['command'] = "菜单指令消息按钮";  
            ...
            $bot_commands2 = Db::name('bot_commands')->where('bot', $data['API_BOT'])->where('command', "设置其它消息按钮")->where('parentId',0)->where('type',0)->find();
            if(empty($bot_commands2)){
                $bot_commands2['bot'] = $data['API_BOT'];  
                $bot_commands2['command'] = "设置其它消息按钮";  
                $bot_commands2['parentId'] =0; 
                $bot_commands2['type'] = 0;  
                $bot_commands2['id'] = Db::name('bot_commands')->insertGetId($bot_commands2);   
                
                #批量初始化其它事件按钮
                $bot_comtype = Db::name('bot_comtype')->where('plugin', "trxbot")->select();  
                $install_bot_commands=[];
                foreach ($bot_comtype as $comVal) {
                    $commands_data['bot'] = $data['API_BOT'];
                    $commands_data['parentId'] = $bot_commands2['id'];
                    $commands_data['command'] = $comVal['command'];  
                    $commands_data['type'] = 2;  
                    $commands_data['chatType'] = $comVal['chatType'];  
                    $commands_data['description'] = $comVal['tips'];  
                    $commands_data['istext'] = $comVal['ismsg'];  
                    array_push($install_bot_commands,$commands_data);
                }
                if(count($install_bot_commands)>0){ 
                    Db::name('bot_commands')->insertAll($install_bot_commands); 
                }
            }
            ...
            if(empty($bot_commands2)){//构建初始化命令  
                ...
                $start['text'] = "本机器人为您提供以下服务：\n1.发送钱包地址可以查询钱包余额情况\n2.发送交易哈希可以查看交易状态详情\n3.给本机器人转USDT可以自动回TRX";   
                $start['photo'] = "https://telegra.ph/file/caa1f5ee9a712397b3ad9.jpg";   
                $start['type'] = 1; 
    ...
    //         return json(['code' => 0,  'msg' => "波场Api错误,点击前面感叹号查看注册流程","find"=>"TRON_API_KEY"]);  
    //     }
        
    //     if($data['TRON_API_KEY'] == getenv('TRONSCAN_APIKEY') ){
    //         return json(['code' => 0,  'msg' => "波场Api错误：你填写了TRONSCAN_APIKEY的APIkey这是2个不同的API哦","find"=>"TRON_API_KEY"]);  
    //     }
