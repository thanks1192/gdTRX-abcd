...
class trx_jt{
    public function  execute(): string{ 
        ...
        if(empty($setup)){ 
            echo "监听TRX出账失败【机器人：{$BOT['API_BOT']}】请登录后台配置钱包信息\n";
            return 'No addr'; 
        }
        ...
        $res =json_decode($client->request('GET', "https://api.trongrid.io/v1/accounts/{$setup['addr']}/transactions?limit=50&only_from=true&min_timestamp={$time}000")->getBody()->getContents(),true); 
        if(!count($res['data'])){
            return 'oka'; 
        } 
        ...
        foreach ($res['data'] as $resval) {
            ...
            $oktime = substr($resval["block_timestamp"],0,10) ?? time() ;
