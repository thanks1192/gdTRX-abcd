...
class usdt_jt{
    public function  execute(): string{  
        ...
        if(empty($setup)){ 
            echo "监听USDT到账失败【机器人：{$BOT['API_BOT']}】请登录后台配置钱包信息\n";
            return 'No addr'; 
        }   
        ...
        if(empty($setup['addr'])){
            ...
            $setup['addr'] = $Account->address()->__toString();
            Db::name('trx_setup')->where("id",$setup["id"])->update(['addr'=>$setup['addr']]);  
            Cache::delete("trx_{$setup['bot']}_setup");
        } 
        ...
        $res =json_decode($client->request('GET', "https://api.trongrid.io/v1/accounts/{$setup['addr']}/transactions/trc20?limit=50&only_to=true&min_timestamp={$time}000&contract_address=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t")->getBody()->getContents(),true);  
        if(!count($res['data'])){
            return 'oka'; 
        } 
        ...
        foreach ($res['data'] as $resval) {  
            ...
            if($LOCK){  
                ...
                if(!empty($total_trc20['tgid'])){ 
                    Db::name('account_tg')->where('tgid',$total_trc20['tgid'])->inc("dhnum",1)->inc("dhusdt",$resval["value"])->update();    
                } 
                //  ---------多表记录更新end-------
                ...
                if($resval["value"] < ($setup['Minusdt']*1000000)){
                 ...
                 $queueData['time'] = substr($resval["block_timestamp"],0,10);
