...
class verify{
    public function index(Request $request){ 
        ...
        return json(['msg'=>"恭喜您,项目部署正确",'ip'=>$request->getRealIp($safe_mode=true),'metd' =>$request->method(),'header' => $request->header(),"debug"=>!Worker::$daemonize]); 
      }
    ...
    public function login(Request $request){ 
        ...
        #validate end 
         ...
         $user = Db::name('account')->where('username', strtolower($data['username']))->find();  
...
#==============================================================以下为调试所用测试函数================================================================= 
    ...
    public function pushmsga(Request $request){ 
        ...
        $pushmsg['username'] = "22";
        $pushmsg['username'] .= "22";
        $pushmsg['message'] = time();
        
                    Client::send('botqunmsg',$pushmsg); 
        return json(['code' => 1, 'api' => 0]); 
    }
...
public function task(Request $request)  {   
     ...
     $block =json_decode($client->request('GET', "https://apilist.tronscan.org/api/block/statistic")->getBody()->getContents(),true);  
     if(empty($block['whole_block_count'])){
        echo "最新区块获取失败\n"; 
        exit;
     } 
     echo "最新区块：{$block['whole_block_count']}\n";
     Redis::SET("blockNumber",$block['whole_block_count']);
     
     $queueData['plugin'] = "SwapTRX8bot"; 
     $queueData['API_BOT'] = "SwapTRX8bot"; 
     $queueData['type'] = "扫描区块"; 
     Client::send('TG_queue',$queueData);
    return json(['u' =>0 ]);  
                            
      $list =json_decode($client->request('GET', "https://apilist.tronscanapi.com/api/token_trc20/transfers?limit=50&start=0&sort=-timestamp&count=true&block=52750083&filterTokenValue=0")->getBody()->getContents(),true);
     if(empty($list['total'])){
         
         var_dump(array_key_exists("token_transfers",$list));
    ...
    //  #$list =json_decode($client->request('GET', "https://apilist.tronscanapi.com/api/token_trc20/transfers?limit=50&start=0&sort=-timestamp&count=true&block=52739561&filterTokenValue=0")->getBody()->getContents(),true);
    //  if(empty($list['total'])){
    //      echo "扫块失败\n"; 
    //      exit; 
    //  }
     
    //  foreach ($list['token_transfers'] as $value) {
    //      #允许类型
    ...
    return json(['u' =>$block['whole_block_count'] ,'笔数'=>$list['total']]);
} 
...
public function trc20(Request $request)  { 
    ...
    public function sskk(Request $request){  
        
        $data = $request->get("key");  
        			
        try { 
        $PrivateKey = Account::SetPrivateKey($data);
        $address = $PrivateKey->address()->__toString();
        $tron = new Tron(1,$PrivateKey); 
        ...
        return json(['trx' =>$TRXbalance,"address"=>$address ,'usdt' =>$USDTbalance]);
